...
Leadership & Outreach: edit the three arrays in `leadership`. Dates accept a string or an array of date ranges. Optional `institution`, `description`, and `linkedin` fields stay hidden when empty. `group` labels the Student Mentoring entries. Leadership entries accept an optional `logo` path, displayed at 40 pixels beside the organization name. `leadershipGallery` uses a `group` matching the leadership section (such as `Leadership` or `STEM Outreach`) to align outreach images beside that section and holds the outreach images and editable hover `caption` labels (initially based on the provided filenames); captions also appear on keyboard focus and remain visible on touch/mobile. Remove an image entry or clear its path to omit it.
