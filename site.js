(() => {
  ...
  const homeLines = (c.profile.homeLines || []).map(line => {
    const characters = Array.from(line);
    const animated = characters.map((character, i) => `<span style="--reveal-delay:${350 + Math.round(i / Math.max(characters.length - 1, 1) * 2400)}ms">${esc(character)}</span>`).join('');
    return `<p class="home-line"><span class="sr-only">${esc(line)}</span><span aria-hidden="true">${animated}</span></p>`;
  }).join('');
  ...
  document.querySelector('#main').innerHTML = `
    <section id="home" class="hero hero-landscape">
      ...
      <div class="hero-copy"><p class="eyebrow">Research & beyond</p><h1>${esc(c.profile.name)}</h1><p class="hero-title">${esc(c.profile.title)}</p><div class="socials">${Object.entries(c.profile.links).map(([name, url]) => safeUrl(url) ? link(name, url, 'social-link') : `<span class="social-pending" title="Link to be added">${esc(name)}<span class="sr-only"> — link to be added</span></span>`).join('')}</div><div class="home-lines">${homeLines}</div></div>
    </section>
    ${section('projects', '01', 'Projects', `<div id="phd-projects" class="subsection"><h3>Ph.D. Projects</h3>${detail(c.profile.bio)}${c.phd.map(project).join('')}</div><div id="masters-project" class="subsection"><h3>Master’s Project</h3>${project(c.masters)}</div><div class="subsection"><h3>Machine Learning Projects</h3><div class="cards">${c.sideProjects.map((p,i) => `<article${i===0?' id="side-project-protein-contact"':''} class="entry side-card">${media(p.image, p.imageAlt, 'Project image to come')}<h4>${safeUrl(p.github) ? link(p.title || 'Project title to come', p.github, 'side-project-link') : esc(p.title || 'Project title to come')}</h4>${detail(p.description)}</article>`).join('')}</div></div>`)}
    ...
    ${section('skills', '04', 'Skills', `<div class="skills-grid"><div><h3>Computational Methods and Theory</h3>${c.skills['Computational Methods and Theory'].map(text => `<p class="skill-line">${skillText(text)}</p>`).join('')}<p class="skill-note">${esc(c.skills.frameworksExposure)}</p></div><div><h3>Software and Languages</h3><p class="skill-line">${skillText(c.skills['Software and Languages'][0])}</p></div><div class="experimental-skills"><h3>Experimental Skills</h3><p class="skill-line">${c.skills['Experimental Skills'].map(skill => internalSkill(skill.label, skill.href)).join('; ')}.</p></div></div>`)}
