(() => {
  ...
  const role = r => `<article${r.id ? ` id="${esc(r.id)}"` : ''} class="entry"><h4>${esc(r.title)}</h4>${detail(r.organization)}${detail(r.institution)}${r.dates?.length ? `<p class="date">${(Array.isArray(r.dates) ? r.dates : [r.dates]).map(esc).join('<br>')}</p>` : ''}${detail(r.description)}${links([['LinkedIn Post', r.linkedin]])}</article>`;
  ...
  document.querySelector('#main').innerHTML = `
    ...
    ${section('leadership', '07', 'Leadership & Outreach', `<div class="section-with-gallery"><div class="leadership-list">${Object.entries(c.leadership).map(([group, entries]) => `<div class="subsection"><h3>${esc(group)}</h3><div class="roles">${roleList(entries)}</div></div>`).join('')}</div>${gallery(c.leadershipGallery, 'Leadership and outreach memories', true)}</div>`)}
